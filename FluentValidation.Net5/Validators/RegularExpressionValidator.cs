...
namespace FluentValidation.Validators {
	...
	public class RegularExpressionValidator : PropertyValidator, IRegularExpressionValidator {
		...
		public RegularExpressionValidator(string expression) : base(() => Messages.RegexError) {
			this.expression = expression;
		}

		public RegularExpressionValidator(Regex regex) : base(() => Messages.RegexError) {
			this.expression = regex.ToString();
			this.regexFunc = x => regex;
		}

		public RegularExpressionValidator(string expression, RegexOptions options) : base(() => Messages.RegexError) {
			this.expression = expression;
			this.regexOptions = options;
		}

		public RegularExpressionValidator(Func<object, string> expression)
			: base(() => Messages.RegexError)
		{
